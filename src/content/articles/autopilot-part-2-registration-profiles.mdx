...
In [Part 1](/deployment/deployment/autopilot-part-1-prerequisites/), we set up our prerequisites. Now it's time to register your first device in Intune and create a deployment profile that tells Autopilot how to configure new PCs.

Let's dive in!

## Understanding the Hardware Hash

Before we register a device, you need to understand what Autopilot actually needs. Each Windows device has a unique identifier called a **hardware hash** â a long string of characters that identifies that specific machine.
...
> ð¡ **Tip:** Think of the hardware hash like a serial number that Intune uses to recognize and claim the device.
...
1. Go to **Devices â Windows â Windows devices** in Endpoint Manager
2. Click **Add** and select **Search OEM devices**
3. The devices will appear â just claim them!
...
This creates a file called `autopilot.csv` in your current folder. Open it â you should see something like:
...
2. Go to **Devices â Windows â Windows devices**
3. Click **Add**
...
> ð¡ **Tip:** You can import multiple devices at once â just add more rows to your CSV file!

## Create an Autopilot Deployment Profile

Now for the magic â the deployment profile tells Autopilot how to set up the device. Let's create one.

1. In Endpoint Manager, go to **Devices â Windows â Autopilot deployment profiles**
...
Click **Next** â **Create** to save your profile.
...
1. Go to **Devices â Windows â Autopilot deployment profiles**
...
4. Choose one of these options:
   - **Select groups** â assign to a specific Azure AD group
   - **All devices** â every Autopilot device gets this profile

> ð¡ **Tip:** For testing, create a small Azure AD group called "Autopilot Test Devices" and assign only a few devices to it. This lets you validate before rolling out to everyone.
...
- â What a hardware hash is
- â How to get the hardware hash using PowerShell
- â How to import devices into Intune
- â How to create an Autopilot deployment profile
- â How to assign the profile to devices

## What's Next?

In [Part 3](/deployment/deployment/autopilot-part-3-testing-troubleshooting/), we'll actually test the deployment, verify the device enrolls correctly, and troubleshoot common issues that might come up.
